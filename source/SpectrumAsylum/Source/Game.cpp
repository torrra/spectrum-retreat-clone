...
\********/
// Create mesh wall
Mesh* Game::CreateWall(const std::string& key, Model* model, const LibMath::Vector3& pos, const LibMath::Vector3& scale, Material* mat, const char* areaKey)
{
...
Mesh* Game::CreateWallWithHole(const std::string& key, Model* model, const LibMath::Vector3& pos, const LibMath::Vector3& scale, Material* mat, const char* areaKey)
{
	// Add wall mesh to the current level
	Mesh*	wall = m_currentLevel.m_scene.AddChild<Mesh>(key, model); 
